...
$_SESSION['Page_Active'] = 'index2.php';
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
if ((isset($_POST['reason'])) and $_POST['reason'] != '') {
    //-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    $query_reason_submit = "SELECT * FROM reason WHERE code = :reason";
    try {
        $stmtsubmit = $connlocal->prepare($query_reason_submit);
        $stmtsubmit->bindParam(':reason', $_SESSION['reason']);
        $stmtsubmit->execute();
        $submitreason = $stmtsubmit->fetch(PDO::FETCH_ASSOC);
        $_SESSION['reason_name'] = $submitreason['LocalName'];
    } catch (PDOException $e) {
        die("Error submitreason " . $e->getMessage());
    }
}
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
$query_reason = "SELECT 
                ...
                ORDER BY code ASC";
try {
    $stmtreason = $connlocal->prepare($query_reason);
    $stmtreason->execute();
    $reason_rows = $stmtreason->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    die("Error stmtclinic " . $e->getMessage());
}
...
if (isset($_SESSION['reason'])) {
    $RowsQuery .= " AND AppmntProcedureCode1 = :reason ";
}
if ((isset($_SESSION['target'])) and (($_SESSION['target'] == 'SC'))) {
    $RowsQuery .= " AND (SELECT STUFF(LocalName, 1, 1, '') FROM dbo.DNSYSCONFIG WHERE CtrlCode = '42203' AND Code = Clinic) LIKE '%SC%' ";
} elseif ((isset($_SESSION['target'])) and ($_SESSION['target'] == 'WI')) {
    $RowsQuery .= " AND (SELECT STUFF(LocalName, 1, 1, '') FROM dbo.DNSYSCONFIG WHERE CtrlCode = '42203' AND Code = Clinic) LIKE '%WI%' ";
} elseif ((isset($_SESSION['target'])) and ($_SESSION['target'] == 'CS')) {
    $RowsQuery .= " AND (SELECT STUFF(LocalName, 1, 1, '') FROM dbo.DNSYSCONFIG WHERE CtrlCode = '42203' AND Code = Clinic) LIKE '%CS%' ";
} elseif ((isset($_SESSION['target'])) and ($_SESSION['target'] == 'Refill')) {
    $RowsQuery .= " AND (SELECT STUFF(LocalName, 1, 1, '') FROM dbo.DNSYSCONFIG WHERE CtrlCode = '42203' AND Code = Clinic) LIKE '%Refill%' ";
}
...
if (isset($_SESSION['reason'])) {
    $totalRowsQuery->bindParam(':reason', $_SESSION['reason']);
}
...
if (isset($_SESSION['reason'])) {
    $query .= " AND AppmntProcedureCode1 = :reason ";
}
if (isset($_SESSION['target']) and $_SESSION['target'] == 'SC') {
    $query .= " AND (SELECT STUFF(LocalName, 1, 1, '') FROM dbo.DNSYSCONFIG WHERE CtrlCode = '42203' AND Code = Clinic) LIKE '%SC%' ";
} elseif (isset($_SESSION['target']) and $_SESSION['target'] == 'WI') {
    $query .= " AND (SELECT STUFF(LocalName, 1, 1, '') FROM dbo.DNSYSCONFIG WHERE CtrlCode = '42203' AND Code = Clinic) LIKE '%WI%' ";
} elseif (isset($_SESSION['target']) and $_SESSION['target'] == 'CS') {
    $query .= " AND (SELECT STUFF(LocalName, 1, 1, '') FROM dbo.DNSYSCONFIG WHERE CtrlCode = '42203' AND Code = Clinic) LIKE '%CS%' ";
} elseif (isset($_SESSION['target']) and $_SESSION['target'] == 'Refill') {
    $query .= " AND (SELECT STUFF(LocalName, 1, 1, '') FROM dbo.DNSYSCONFIG WHERE CtrlCode = '42203' AND Code = Clinic) LIKE '%Refill%' ";
}
...
try {
    ...
    if (isset($_SESSION['reason'])) {
        $stmt->bindParam(':reason', $_SESSION['reason']);
    }
    $stmt->execute();
    ...
    <section id="portfolio" class="portfolio" style="padding-top: 150px;">
        <div class="container-fluid" style="padding-left: 50px;padding-right: 50px;">
            ...
            <form action="" method="post">
                <nav class="navbar navbar-expand-md bg-body border rounded-0 py-3" style="margin-bottom: 10px;">
                        ...
                        <div class="collapse navbar-collapse" id="navcol-2">
                            <ul class="navbar-nav ms-auto">
                                ...
                                <li class="nav-item" style="margin-right: 5px;padding-top: 5px;padding-bottom: 0px;">
                                    <label class="form-label" style="margin-right: 5px;margin-bottom: 0px;">
                                        ClinicGroup
                                        <select name="ClinicGroup" class="form-select">
                                ...
                                <li class="nav-item" style="margin-right: 5px;padding-top: 5px;padding-bottom: 0px;">
                                    <label class="form-label" style="margin-right: 5px;margin-bottom: 0px;">
                                        ...
                                        <select name="reason" class="form-select">
                                            <option value=""><?= $_SESSION['reason_name'] ?><?php if ($_SESSION['reason_code'] == '' or $_SESSION['reason_code'] == null) echo 'เลือก เหตุผลการนัด' ?></option>
                                            <?php foreach ($reason_rows as $row) { ?>
                                                <!-- <option value="<?= $row['code'] ?>"><?= $row['code'] . ' ' . ' ' . $row['LocalName'] ?></option> -->
                                                <option value="<?= $row['code'] ?>"><?= $row['LocalName'] ?></option>
                                            <?php } ?>
                                        </select>
                                    </label>
                                </li>
                                <li class="nav-item" style="margin-right: 5px;padding-top: 5px;padding-bottom: 0px;">
                                    <label class="form-label" style="margin-right: 5px;margin-bottom: 0px;">
                                        ...
                                        <select name="target" class="form-select">
                                            <option value="">เลือกกลุ่มคนไข้</option>
                ...
                <div class="col-12">
                    <div class="row d-flex justify-content-end">
                        <div class="col">
                            <div class="text-start">
                                ...
                                <h4>target : <?= $_SESSION['target'] ?> reason : <?= $_SESSION['reason'] . ' ' . $_SESSION['reason_name'] ?></h4>
                                <h4>วันที่ : <?= $_SESSION['startdate'] ?> ถึงวันที่ : <?= $_SESSION['enddate'] ?></h4>
                            </div>
                        </div>
                        <div class="col">
                            <div class="text-end">
                                <button onclick="exportToExcel()" type="button" class="btn btn-dark" style="margin-top: 10px;margin-left: 10px;margin-right: 10px;margin-bottom: 10px;">export</button>
                            </div>
                        </div>
                    </div>
                                                        ...
                                                        <div class="container text-start">
                                                            <div class="row">
                                                                <div class="col-lg-8 mx-auto">
                                                                    ...
                                                                    <input type="radio" class="btn-check" name="status" id="success-outlined" autocomplete="off" value="รับสาย" checked>
                                                                    <label class="btn btn-outline-success" for="success-outlined">รับสาย</label>
                                                                    <input type="radio" class="btn-check" name="status" id="danger-outlined" autocomplete="off" value="ไม่รับสาย">
                                                                    <label class="btn btn-outline-danger" for="danger-outlined">ไม่รับสาย</label>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                    ...
                    <?php if ($totalRows > 0) { ?>
                        <div class="pagination">
                            ...
                            if ($totalPages > 1) {
                                if ($current_page > 1) {
                                    echo "<a href='index2.php?page=1'>Home</a>";
                                    echo "<a href='index2.php?page=" . ($current_page - 1) . "'>Previous</a>";
                                }

                                for ($i = $start_page; $i <= $end_page; $i++) {
                                    echo "<a href='index2.php?page=$i'";
                                    if ($i == $current_page) {
                                        echo " class='active'";
                                    }
                                    echo ">$i</a>";
                                ...
                                if ($current_page < $totalPages) {
                                    echo "<a href='index2.php?page=" . ($current_page + 1) . "'>Next</a>";
                                    echo "<a href='index2.php?page=$totalPages'>Last Page</a>";
                                }
                            }
                            ?>
                        </div>
                ...
                foreach ($hidenrows as $row) { ?>
                    <tr>
                        ...
                        <?php if ($row['NationCASE'] == 'th') { ?>
                            <td><?= $row['InitialName'] ?> <?= $row['FirstName'] . ' ' . $row['LastName'] ?> มีนัดพบ <?= $row['DoctorName'] ?> แผนก <?= str_replace(['(SC).', '(SC)', '(CS)', '(WI)'], '', $row['Clinic']) ?> โรงพยาบาลเกษมราษฎร์ รามคำแหง วันที่ <?= convertToThaiDate(getOnlyDay($row['AppointDateTime'])) ?> เวลา <?= getOnlyTime($row['AppointDateTime']) ?> น.</td>
                        <?php } else { ?>
                            <td><?= $row['InitialName'] ?> <?= $row['FirstName'] . ' ' . $row['LastName'] ?> has an appointment with Dr.<?= $row['DoctorName'] ?> at the <?= str_replace(['(SC).', '(SC)', '(CS)', '(WI)'], '', $row['Clinic']) ?> Department, Kasemrad Hospital Ramkhamhaeng, on <?= convertToEngDate(getOnlyDay($row['AppointDateTime'])) ?>,at <?= getFormattedTime($row['AppointDateTime']) ?>.</td>
                        <?php } ?>
                    </tr>
                <?php }
            } else { ?>
